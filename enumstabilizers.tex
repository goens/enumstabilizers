...
\item $V \otimes W$ the tensor-product of $V$ and $W$.
...
With the notation from def. \ref{traces}, the partial trace of $A \in \End(V \otimes W)$ Hermitian, is the unique Hermitian operator $\Tr_W(A) \in \End(V)$ which satisfies:
 \begin{equation} \Tr(A(B \otimes \Id_W)) = \Tr(\Tr_W(A)B) \forall B \in \End(V) \text{ Hermitian} \end{equation}
Further for a Hilbert space with a particular decomposition $V_1 \otimes \ldots V_n, \text{e.g.} (\C^2)^{\otimes n} \text{with } V_i \cong \C^2 \, \forall \, i \, \in \n$, and $S \dot{\cup} S^c = \underline{n}, S = \{ s_1 < \ldots < s_j \}, S^c = \{ s^c_1 < \ldots < s^c_k \}$ we define $V_S := V_{s_1} \otimes \ldots \otimes V_{s_j}, V_{S^c} := V_{s^c_1} \otimes \ldots \otimes V_{s^c_k}$
And use $\Tr_S$  as a short notation for $\Tr_{V_S} : \End(V_1 \otimes \ldots \otimes V_n) \rightarrow \End(V_{S^c})$.
...
\begin{proof}
$ \Tr_W(A)$ is hermitian since $A$ already is. Let $ A = \sum_{i=1}^k V_i \otimes W_i$ with $V_i \in \End(V), W_i \in \End(W)$ Hermitian $\fa i \in \underline{k}$. Then $\Tr_W(A) = \sum_{i=1}^k \Tr(W_i) V_i$ satisfies  $\forall B \in \End(W) \text{Herm.}$:
...
the set of all so-called Pauli operators, and by $\mathcal{P}_n:= \langle \mathcal{E} \rangle \leq \GL_n(\C)$, called the so-called Pauli group. 
\end{defn}

\begin{rem}
The set $\{ \Id, X, Y, Z \}$ is linearly independent and thus forms a basis of $\C^{2 \times 2}$. A simple calculation shows that this basis is orthogonal with respect to the trace bilinear form on $\C^{2 \times 2}$. Analogously, since $\Tr(A \otimes B) = \Tr(A) \Tr(B) \forall A \in \C^{m \times m}, B \in \C^{m' \times m'}$, $\mathcal{E}$ is an orthogonal basis of $\C^{2^n \times 2^n}$.
\end{rem}
...
%% Let G be a finite p-group, i.e. $p | ord(g) \, \forall \, g \in G$. Then the following is true: 
%% \begin{enumerate}
...
\begin{enumerate}
\item $Z(G) = \{ \pm \Id, \pm i \Id \}$, where $Z(G)$ is the centre of G.
\item $G' = \pm \{ \Id \} \leq Z(G)$, where $G'$ is the commutator subgroup of G.
\item $G/Z(G)$ is a $2n$-dimensional $\F_2$ vectorspace
\item The commutator $[\cdot,\cdot] : G \times G \rightarrow G$ induces a non-degenerate symmplectic product on $G/Z(G)$
\end{enumerate}
...
\item Since $G' \leq Z(G)$ the induced mapping is well-defined. Since $[g,g] = 1 \fa g \in G$ it follows that $[,]$ is alternating on $G/Z(G)$. $\F_2$ bilinearity follows directly from the definition.
\end{enumerate}
\end{proof}
\end{prop}
...
Let $U$ denote the the group of unitary $n \times n$ matrices over the complex field $\C$. 
Then there exists a unique measure $\mu$ on $U$, called the Haar measure, which satisfies the following:
\begin{itemize}
\item $\mu$ is a countably additive, non-trivial meassure on the Borel subsets of U (i.e. on the $\sigma-$Algebra generated by the open subsets of U). 
\item $\mu(U) = 1$
...
\begin{rem}
The Haar measure makes the group of unitary $n \times n$ matrices $U$ over $\C$ with its Borel sets into a measurable space, and through it, all familiar concepts of stochastics carry over.
...
\begin{theorem}
Let $U$ be a random unitary $n \times n$ matrix over $\C$ and $A,B \in \C^{n \times n}$. Then:
...
For the first one (eqn. \ref{fact2}) we note that $\text{for }W \in \U_n(\C):$
 \begin{equation}  W E(UAU^{\dagger}) W^\dagger = W \int_{\U} ( U A  U^\dagger) d \mu (U) W^\dagger = \int{\U} (WU) A (WU)^\dagger d \mu (WU) = E(UAU^\dagger) \end{equation}, where in the second equality we used the fact that $\mu$ is translation invariant.
...
Since $B = B^\dagger = B^{-1} \fa B \in \E \subseteq \U_n$. But $ \langle \E \rangle_\C = \C^{n \times n}$, so it holds for all $B \in \C^{n \times n}$ from the bilinearity of $\psi$. That $\lambda = \frac 1 n$ follows again from $\psi(I_n,I_n) = \int_{\U_n} \underbrace{\Tr(U) \Tr(U^{\dagger})}_{= |\Tr(U)|^2 = 1} d \mu (U) = 1$.
\end{proof}
\end{theorem}
...
Because of its many properties, as discussed in section \ref{paulisection}, we will first consider the pauli operators ($\E$) as errors. These are, in particular, a unitary, hermitian basis of $\End(\H)$. There is also a very good physical motivation for choosing the pauli operators, since these are closely related to the spin of the system.
...
With this in mind, it is not difficult to see why coding $ |0\rangle \mapsto | +++ \rangle$,  $|1 \rangle \mapsto | --- \rangle$ would be a good idea. If we make the same measurement as in example  \ref{repetitionqubits}, but in the $ | + \rangle, | - \rangle$ basis instead, 
...
Let $\{ E_i \mid i \in I \}$ for some index set $I$ be a set of unitary operators, and $C$ be a quantum code that can correct errors on all these $E_i, i \in I$. Further let $\{ F_j \mid j \in J \}$ for another index set $J$ be another set of unitary operators, which is a linear combination of the $E_i$, i.e. $ F_j = \sum_{i \in I} c_{i,j} E_i$ for all $j \in J$ and for some $c_{i,j} \in \C$, almost all $c_{i,j} = 0$.
Then, $C$ can also correct all errors $F_j, j \in J$. This theorem actually holds for a wider class of errors, where $E_i, F_j$ are quantum operations; but this goes beyond the scope of this thesis. The more general statement, along with a proof, can be found in \cite{NC}.
\end{theorem}
...
%% \begin{defn}
%% Let $S \leq \mathcal{P}_n $ be a stabilizer group. Any $A \in S$ as in remark  \ref{Pauliprops} can be written as $ A = i^j X_1^{a_1} Z_1^{b_1} X_2^{a_2} Z_2^{b_2} \ldots X_n^{a_n} Z_n^{b_n}$, where $j \in \{ 0, 1 , 2 , 3 \}$, $a_i, b_i \in \{ 0 , 1 \} \, \forall \, i \in \underline{n}$. 
%% Then the mapping
...
\begin{defn}
Let $S \leq \P_n$ be a stabilizer group. The centralizer of S in $\P_n$, $C_{\P_n}(S)$ is defined as:
...
Logical operators have the following physical interpretation:
...
With a little more mathematics (the development of which would be too long for this thesis), logical operators can be further understood: 

\begin{rem}
FIXME: prove/check this:
For each operator $A$ on $k$ qubits, there is - of course - an operator $A^{(n)}$ on the encoding $n$ qubits. This operator is not unique however: an equivalency class can be defined for logical operators, making two operators equivalent, when they act as the same operator on the $k$ logical qubits.
Two such equivalent operators differ actually always by an element of the stabilizer group, i.e. : $ A^{(n)}$ and $ B^{(n)}$ are equivalent iff $ \exists T \in S$ with $ A^{(n)} = B^{(n)} T$. That these is true can be made at least plausible by reminding us of the definition of the stabilizer group: it does not
alter the code!
\end{rem}
...
Let $ E \in \P_n$ be an arbitrary Pauli operator. The weight of E, $ | E |$, is defined as the number of qubits on which $E$ is supported, i.e. the number of qubits on which $E$ acts non-trivially, while disregarding an overall phase (factor).
...
\begin{rem}
  The above defined $C = C(D_1,D_2)$ is an $[[n,k_1 - k_2,d]]$ code, with $d \geq 2t+1$. FIXME: proof!!  A proof can be found in \cite{NC}.
 \end{rem}
...
This definition is consistent with the weight on $\F_4^n$.

\begin{defn}\label{shorlaflamme}
Let $V = (\C^2)^{\otimes n}$, and $\mathcal{E} = \{ \sigma_1 \otimes \ldots \otimes \sigma_n \mid \sigma_i \in \{X,Y,Z,\Id \} \forall i \in \underline{n} \}$ (generating set for the pauli group on V). Further let $S \subseteq \underline{n}$ and $M_1, M_2 \in \C^{2^n \times 2^n} (\cong \text{End}(V))$, with $\Tr(M_1) \neq 0 \neq \Tr(M_2)$.
...
Again let $V = (\C^2)^{\otimes n}$, and $\mathcal{E} = \{ \sigma_1 \otimes \ldots \otimes \sigma_n \mid \sigma_i \in \{X,Y,Z,\Id \} \forall i \in \underline{n} \}$. Further let $S \subseteq \underline{n}, S^c = \n \backslash S$ and $M_1, M_2 \in \C^{2^n \times 2^n}$.
We define the coefficient of the Rains enumerators \cite{rains} of $M_1,M_2$ as follows:
\begin{equation}A'_S(M_1,M_2) = \Tr(\Tr_{S^c}(M_1)\Tr_{S^c}(M_2)) \end{equation}
\begin{equation}B'_S(M_1,M_2) = \Tr(\Tr_S(M_1)\Tr_S(M_2)) \end{equation}
...
Let $C \leq (C^2)^{\otimes n} =: V$ be an additive quantum code and $G \subseteq \mathcal{E}$ be its stabilizer group (i.e. $C = V^G$). and $E,E' \in \mathcal{E} = \{ \sigma_1 \otimes \ldots \otimes \sigma_n \mid \sigma_i \in \{X,Y,Z,\Id \} \forall i \in \underline{n} \}$. Then
\begin{enumerate}
...
\item $\Tr(\Tr_S(E)\Tr_S(E')) = 0$ if $E \neq E'$ or if there exists an $i \in S$ which satisfies that for $E = \sigma_1 \otimes \ldots \sigma_n$, $\sigma_i \neq \Id$, otherwise $\Tr(\Tr_S(E)\Tr_S(E)) = 2^n$.
\item $\Tr( E P_C E P_C) = \left{ \begin{array}{lr} \frac{2^n}{|G|} & \text{ for } E \in G^\perp \\ 0 & \text{ otherwise}$
\end{enumerate}

\begin{bew}
\begin{enumerate}
\item From a simple calculation it follows that $\sigma P_c(v) = P_c(v)$ for all $v \in V, \sigma \in G$ and $P_c(v) = v \, forall \, v \in V^G$. From this it follows immediately that Im$(P_C) = V^G$ and $P_C^2 = P_C$, and hence that it is the projection onto $V^G$.
...
On the other hand, if $E = E'$ and $\supp(E) \cap S = \emptyset$, then $\Tr_S(E)\Tr_S(E) = 2^{|S|} \Id_{S^c}$ and $2^{|S|} \Tr(\Id_{S^c}) = 2^{|S|} \dim(V_{S^c}) = 2^n$.
...
\begin{theorem}
Let $C \leq (C^2)^{\otimes n} =: V$ be an additive quantum code and $G \subseteq \mathcal{E}$ be its stabilizer group (i.e. $C = V^G$), and let $P_C$ be the projection onto $C$, and let $S \subseteq \n$. Then the coefficients are given as follows:
...
\begin{equation} A'_S(P_C,P_C) = \frac{2^n}{|G|^2} \# \{ \sigma \in G \mid \sigma_{\big| S^c} = \Id \} \end{equation}
\begin{equation} B'_S(P_C,P_C) = \frac{2^n}{|G|^2} \# \{ \sigma \in G \mid \sigma_{\big| S} = \Id \} \end{equation}
...
\begin{equation} = \underbrace{(\frac{2^n}{ |G| \dim C})^2 }_{ = 1, \text{see lemma on dim C}} \# \{ E \in G \mid \supp(E) = S \} \end{equation}
...
\begin{equation} A'_S(P_C,P_C) = \Tr(\Tr_{S^c}(P_C)\Tr_{S^c}(P_C)) = \frac{1}{|G|^2} \sum_{\sigma, \sigma' \in G}  \Tr(\Tr_{S^c}(\sigma)\Tr_{S^c}(\sigma')) \end{equation}
\begin{equation} = \frac{1}{|G|^2} \sum_{\sigma \in G}  \Tr(\Tr_{S^c}(\sigma)\Tr_{S^c}(\sigma)) = \frac{2^n}{|G|^2} \# \{ \sigma \in G \mid \sigma_{\big| S^c} = \Id \} \end{equation}
Again, the 2. and 3. equalities follow from lemma \ref{lemmaenums}. For $B'_S$ it is enough to see that $B'_{S^c}(M_1,M_2) = A'_S(M_1,M_2)$ for all $M_1, M_2$ hermitian directly from its definition.
\end{proof}
...
There is an equivalent definition of the Rains enummerators, which also sheds some light to the motivation for defining $A'$ and $B'$. 
...
\[ A'_S = \frac{2^n}{|G|^2} \#\{ \sigma \in G \mid \sigma_{\big| S^c} = \Id \} = \frac{2^n}{|G|^2} \sum_{T \subseteq} \underbrace{ \# \{ \sigma \in G \mid \supp(\sigma)=T \} }_{= A_T} = \frac{2^{n}}{|G|^2} \sum_{T \subseteq S} A_T \]
Now for $B'_S$ we use theorem \ref{expectedvalues}:
\[ B'_S = 2^{|S|} E( \Tr( P_C (U_s \otimes \Id_{S^c}) P_C (U_S \otimes \Id_{S^c}) ) ) = \frac{2^{|S|}}{|G|^2} \sum_{g,g' \in G} E( \Tr[ g (\sigma U_S \otimes \Id_{S^c}) g' (U^\dagger_S \otimes \Id_{S^c}) ] ) \]
Using the expansion for in the orthogonal basis $\E: U_S \otimes \Id_{S^c} = \sum_{\sigma \in \E} 2^{n} \Tr( (U_S \otimes \Id_{S^c}) \sigma) \sigma$ yields: 
\[B'_S = \frac{2^{|S|-2n}}{|G|^2} \sum_{g,g' \in G} \sum_{\sigma, \sigma' \in \E} E( \Tr[ g \Tr(\sigma (U_S \otimes \Id_{S^c}))\sigma  g' \Tr(\sigma' (U_S^\dagger \otimes \Id_{S^c}))\sigma' ] ) \]
\[  = \frac{2^{|S|-2n}}{|G|^2} \sum_{g,g' \in G} \sum_{\sigma, \sigma' \in \E} \Tr( g \sigma  g' \sigma')  E[ \Tr(\sigma (U_S \otimes \Id_{S^c})) \Tr(\sigma (U_S^\dagger \otimes \Id_{S^c})) ]  \]
Lemma \ref{lemmaenums} together with equation \ref{fact1} yield $\ E[ \Tr((U_S \otimes \Id_S^c)\sigma) \Tr((U_S^\dagger \otimes \Id_S^c)\sigma' )] = 2^{n-|S|} \delta_{\sigma,\sigma'}$, from which in turn follows that:
\[ B'_S = 2^{-n} \sum_{\sigma \in \E} \frac{1}{|G|^2} \sum_{g,g' \in G} \Tr(g \sigma g' \sigma) = 2^{-n} \sum_{\sigma \in \E} \Tr( P_C \sigma P_C \sigma) = 2^{|S| - n} B_S \]
\end{bew}
\end{prop}

\begin{cor}\label{corlincomb}
for the d's and mcwilliams 1.
\end{cor}
