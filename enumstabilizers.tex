...
From this we can read $B_d$ as the coefficient of $x^{n-d}y^d$ to be the Krawtchouk polynomial.
\end{theorem}
...
fulfils $A(x,y) = A(x,-y)$ we get the required identity from the MacWilliams transform. Hence, we assume $G$ has elements of odd weight.
...
$S(C)$ is exactly $G_0^\perp \backslash G^\perp$. Using the combinatorial intepretation of the weight enumerators (as hamming weight enumerators), we can thus express the shadow enumerator:
\begin{equation}
